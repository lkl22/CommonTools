...
class CipherUtil:
    @staticmethod
    def encrypt(data, key: bytes, encryptType="AES"):
        """
        对数据进行加密
        :param data: 原始数据
        ...
        try:
            if encryptType == "AES":
                encryptObj = AES.new(key, AES.MODE_ECB)  # ECB模式
                k = 16
            else:
                encryptObj = DES.new(key, DES.MODE_ECB)  # ECB模式
                k = 8
    ...
    @staticmethod
    def decrypt(data, key: bytes, decryptType="AES"):
        """
        对数据进行解密
        :param data: 加密后的数据
        :param key: 加解密数据时的key
        ...
        try:
            if decryptType == "AES":
                decryptObj = AES.new(key, AES.MODE_ECB)  # ECB模式
            else:
                decryptObj = DES.new(key, DES.MODE_ECB)  # ECB模式
